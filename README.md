...
任务状态保存在 FastAPI 进程内存中，服务重启后历史任务会清空。应用会把兑换码按每 20 个一批串行执行，默认单批最长等待 10 分钟。任务开始时的额度预检属于可降级步骤，默认最多等待 15 秒；超时后会直接继续兑换，避免任务长时间停留在 4%。批量接口返回的卡密级错误会按原因汇总展示，且不会在日志中暴露卡密原文。

可通过环境变量调整预检等待时间：

```bash
ACCOUNT_IMPORT_HEALTH_TIMEOUT=15 \
  uv run uvicorn app.main:app --host 0.0.0.0 --port 8000
```
