...
也可以使用“401 自动找回”：先选择一个 Sub2API 分组并扫描账号状态，再对带有兑换码的 401 账号执行找回。新账号从备注读取兑换码，旧格式账号兼容名称末尾的兑换码；找回结果按兑换码和邮箱匹配后原地更新，因此会保留原账号 ID、分组、代理和历史用量。扫描读取的是 Sub2API 已记录的错误状态，不会主动向所有上游账号发送测试请求；HTTP 402 等非 401 异常会单独计数，不会进入自动找回。
